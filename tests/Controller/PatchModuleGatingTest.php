<?php

declare(strict_types=1);

namespace App\Tests\Controller;

use App\Entity\Patch;
use App\Entity\Tenant;
use App\Entity\User;
use App\Service\ModuleConfigurationService;
use Doctrine\ORM\EntityManagerInterface;
use PHPUnit\Framework\Attributes\AllowMockObjectsWithoutExpectations;
use PHPUnit\Framework\Attributes\Test;
use Symfony\Bundle\FrameworkBundle\KernelBrowser;
use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;

/**
 * Verifies that PatchController correctly enforces the `patches` module gate.
 *
 * Two scenarios per endpoint:
 *   module OFF → authenticated user is redirected (302) to the dashboard
 *   module ON  → authenticated user receives a 200 OK response
 *
 * Guards against regressions in module-gating enforcement for Patch Management
 * (NIS2 Art. 21.2(e) + ISO 27001 A.8.8).
 */
#[AllowMockObjectsWithoutExpectations]
class PatchModuleGatingTest extends WebTestCase
{
    private KernelBrowser $client;
    private EntityManagerInterface $entityManager;
    private ?Tenant $testTenant = null;
    private ?User $testUser = null;
    private ?Patch $testPatch = null;

    protected function setUp(): void
    {
        $this->client = static::createClient();
        $this->client->disableReboot();

        $container = static::getContainer();
        $this->entityManager = $container->get(EntityManagerInterface::class);

        // Ensure setup-complete lock exists — without it SetupRequiredSubscriber
        // redirects every authenticated request to /de/setup/.
        $lockFile = $container->getParameter('kernel.project_dir') . '/config/setup_complete.lock';
        if (!file_exists($lockFile)) {
            @file_put_contents($lockFile, date('c'));
        }

        $this->createTestData();
    }

    protected function tearDown(): void
    {
        foreach ([$this->testPatch, $this->testUser, $this->testTenant] as $entity) {
            if ($entity === null) {
                continue;
            }
            try {
                $managed = $this->entityManager->find($entity::class, $entity->getId());
                if ($managed !== null) {
                    $this->entityManager->remove($managed);
                }
            } catch (\Exception) {
            }
        }

        try {
            $this->entityManager->flush();
        } catch (\Exception) {
        }

        parent::tearDown();
    }

    // -------------------------------------------------------------------------
    // Helper: swap ModuleConfigurationService in the test container
    // -------------------------------------------------------------------------

    /**
     * @param list<string> $activeModules Module keys that should appear active.
     */
    private function mockModules(array $activeModules): void
    {
        $mock = $this->createMock(ModuleConfigurationService::class);
        $mock->method('isModuleActive')->willReturnCallback(
            fn(string $key): bool => in_array($key, $activeModules, true)
        );
        static::getContainer()->set(ModuleConfigurationService::class, $mock);
    }

    /** All required + optional modules active, including patches. */
    private function moduleOn(): void
    {
        $this->mockModules([
            'core', 'authentication', 'audit_logging', 'documents', 'objectives', 'workflows',
            'assets', 'risks', 'controls', 'incidents', 'audits', 'training', 'reviews',
            'bcm', 'compliance', 'vulnerability_intel', 'patches',
        ]);
    }

    /** Required modules active but patches explicitly absent. */
    private function moduleOff(): void
    {
        $this->mockModules([
            'core', 'authentication', 'audit_logging', 'documents', 'objectives', 'workflows',
            'assets', 'risks', 'controls', 'incidents', 'audits', 'training', 'reviews',
            'bcm', 'compliance',
            // patches intentionally absent
        ]);
    }

    private function createTestData(): void
    {
        $uid = uniqid('patch_gate_', true);

        $this->testTenant = new Tenant();
        $this->testTenant->setName('ModuleGate Tenant ' . $uid);
        $this->testTenant->setCode('mg_' . substr($uid, -8));
        $this->entityManager->persist($this->testTenant);

        $this->testUser = new User();
        $this->testUser->setEmail('mg_user_' . $uid . '@example.com');
        $this->testUser->setFirstName('Gate');
        $this->testUser->setLastName('Tester');
        $this->testUser->setRoles(['ROLE_USER']);
        $this->testUser->setPassword('hashed_password');
        $this->testUser->setTenant($this->testTenant);
        $this->testUser->setIsActive(true);
        $this->entityManager->persist($this->testUser);

        $this->testPatch = new Patch();
        $this->testPatch->setTenant($this->testTenant);
        $this->testPatch->setPatchId('PATCH-GATE-' . $uid);
        $this->testPatch->setTitle('Gate Test Patch ' . $uid);
        $this->testPatch->setDescription('Module gating test fixture');
        $this->testPatch->setVendor('TestVendor');
        $this->testPatch->setProduct('TestProduct');
        $this->testPatch->setPatchType('security');
        $this->testPatch->setPriority('medium');
        $this->testPatch->setStatus('available');
        $this->testPatch->setReleaseDate(new \DateTimeImmutable());
        $this->entityManager->persist($this->testPatch);

        $this->entityManager->flush();
    }

    // =========================================================================
    // PatchController — module OFF (should redirect)
    // =========================================================================

    #[Test]
    public function patchIndexRedirectsWhenModuleOff(): void
    {
        $this->moduleOff();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch');
        $this->assertResponseRedirects();
    }

    #[Test]
    public function patchNewRedirectsWhenModuleOff(): void
    {
        $this->moduleOff();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch/new');
        $this->assertResponseRedirects();
    }

    #[Test]
    public function patchShowRedirectsWhenModuleOff(): void
    {
        $this->moduleOff();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch/' . $this->testPatch->getId());
        $this->assertResponseRedirects();
    }

    #[Test]
    public function patchEditRedirectsWhenModuleOff(): void
    {
        $this->moduleOff();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch/' . $this->testPatch->getId() . '/edit');
        $this->assertResponseRedirects();
    }

    // =========================================================================
    // PatchController — module ON (should succeed)
    // =========================================================================

    #[Test]
    public function patchIndexSucceedsWhenModuleOn(): void
    {
        $this->moduleOn();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch');
        $this->assertResponseIsSuccessful();
    }

    #[Test]
    public function patchNewSucceedsWhenModuleOn(): void
    {
        $this->moduleOn();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch/new');
        $this->assertResponseIsSuccessful();
    }

    #[Test]
    public function patchShowSucceedsWhenModuleOn(): void
    {
        $this->moduleOn();
        $this->client->loginUser($this->testUser);
        $this->client->request('GET', '/en/patch/' . $this->testPatch->getId());
        $this->assertResponseIsSuccessful();
    }
}
