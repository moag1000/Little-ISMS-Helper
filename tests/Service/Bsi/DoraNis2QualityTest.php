...
final class DoraNis2QualityTest extends TestCase
{
    ...
    // ── INTEGRITY: DORA→NIS2 lex_specialis_candidate is NOT amtlich ──────────
    // DORA pairings do NOT blanket-resolve to amtlich (no verbatim official table exists).
    // Their tier comes from the panel verdicts (ki_validiert / deprecated), not from provenance.

    #[Test]
    public function lexSpecialisCandidateProvenanceDoesNotResolveToAmtlich(): void
    {
        $m = new ComplianceMapping();
        $m->setProvenanceSource('lex_specialis_candidate');

        self::assertNotSame(
            IsoToBsiGapService::TIER_AMTLICH,
            $this->trustService->trustOf($m),
            'lex_specialis_candidate must NOT resolve to TIER_AMTLICH (DORA integrity fix)',
        );
    }

    #[Test]
    public function lexSpecialisCandidateProvenanceIsNotInOfficialSources(): void
    {
        self::assertNotContains(
            'lex_specialis_candidate',
            IsoToBsiGapService::PROVENANCE_OFFICIAL_SOURCES,
            'lex_specialis_candidate must NOT be listed in PROVENANCE_OFFICIAL_SOURCES',
        );
    }
