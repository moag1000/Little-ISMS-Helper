 ...
 *  3. provenanceSource is marked 'lex_specialis_candidate' (NOT amtlich — integrity fix:
 *     DORA↔NIS2 pairings are NOT blanket-amtlich; tier from panel verdicts).
...
final class DoraLexSpecialisMappingAlignmentTest extends TestCase
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Guard 3 — DORA integrity: provenanceSource = 'lex_specialis_candidate' (NOT amtlich)
    // Rationale: no verbatim official pairing table exists; tier comes from panel verdicts.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    #[Test]
    #[DataProvider('lexSpecialisFileProvider')]
    public function provenance_source_is_lex_specialis_candidate(
        string $file,
        string $sourceFramework,
        string $targetFramework,
        string $doraSide,
        ...
        self::assertSame(
            'lex_specialis_candidate',
            $actual,
            sprintf(
                '%s: library.provenance.provenanceSource must be "lex_specialis_candidate" '
                . '(NOT "official_eu_lex_specialis" — DORA integrity fix). Got: "%s". '
                . 'DORA↔NIS2 pairings are NOT blanket-amtlich; their tier comes from panel verdicts.',
                basename($file),
                $actual,
            ),
        );
    ...
    /**
     * Guard: provenanceSource must NOT be 'official_eu_lex_specialis' (amtlich override removed).
     *
     * @param 'source'|'target' $doraSide
     * @param 'source'|'target' $nis2Side
     */
    #[Test]
    #[DataProvider('lexSpecialisFileProvider')]
    public function provenance_source_is_not_official_eu_lex_specialis(
        string $file,
        string $sourceFramework,
        string $targetFramework,
        string $doraSide,
        ...
        self::assertNotSame(
            ...
            $actual,
            sprintf(
                '%s: provenanceSource must NOT be "official_eu_lex_specialis" (DORA integrity fix). '
                . 'Got: "%s".',
                basename($file),
                $actual,
            ),
