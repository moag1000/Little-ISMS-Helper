...
/**
 * Seeds ISO 27001:2022 Annex A (93 Controls) into a tenant's control
 * table. The SoA UI must never be empty when the controls module is
 * active — this service closes that gap by upserting the canonical
 * Annex-A dataset per tenant on demand (idempotent).
 *
 * Called from:
 *   - AdminDashboardController "fix" action when the alert fires
 *   - StatementOfApplicabilityController byCategory() when the tenant
 *     has no controls yet (auto-bootstrap so the user never sees an
 *     empty category page)
 *   - LoadAnnexAControlsCommand legacy CLI entry
 */
class AnnexAControlsBootstrapService
{
    ...
    /**
     * Ensure all 93 Annex-A controls exist for the given tenant.
     * Existing rows (matched on controlId + tenant) are left alone —
     * no downgrading of isApplicable / implementationStatus.
     *
     * @return int Count of newly-created controls for this run
     */
    public function ensureLoadedForTenant(Tenant $tenant): int
    {
        $created = 0;
        foreach ($this->getAnnexAControls() as $data) {
            $existing = $this->controlRepository->findOneBy([
                'tenant' => $tenant,
    ...
    /**
     * True when the tenant has at least one Annex-A control (tenant-
     * scoped). Used by the admin dashboard alert and the SoA empty-
     * state gate.
     */
    public function isTenantSeeded(Tenant $tenant): bool
    {
        return $this->controlRepository->count(['tenant' => $tenant]) > 0;
    }

    /**
     * @return array<int, array{controlId: string, name: string, description: string, category: string}>
     */
    private function getAnnexAControls(): array
    {
        return [
            ...
            ['controlId' => 'A.5.22', 'name' => 'Monitoring, review and change management of supplier services', 'description' => 'The organization shall regularly monitor, review, evaluate and manage changes in supplier information security practices and service delivery.', 'category' => 'Organizational'],
            ...
            ['controlId' => 'A.8.15', 'name' => 'Logging', 'description' => 'Logs that record activities, exceptions, faults and other relevant events shall be produced, stored, protected and analysed.', 'category' => 'Technological'],
