...
/**
 * SetupBaselineApplier — extracted from SetupRecommendationEngine (arch followup).
 *
 * Owns the infrastructure-heavy "apply starter baseline" action that requires
 * Security (current user resolution), KernelInterface (console command bootstrap
 * for on-demand baseline seeding), and EntityManagerInterface (identity-map clear).
 *
 * These are request-bound / infrastructure concerns that do NOT belong in the
 * pure-computation SetupRecommendationEngine. Extracting them here lets
 * SetupRecommendationEngine remain a stateless array-computation service.
 *
 * Public API:
 *   - applyGenericStarterBaseline() → string  (human-readable result message)
 */
