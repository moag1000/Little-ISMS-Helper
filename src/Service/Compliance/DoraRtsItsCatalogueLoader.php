...
/**
 * Reusable loader for the DORA Level-2 catalogue (RTS / ITS / CIRs / Joint
 * Guidelines) issued by the ESAs Joint Committee under DORA (Regulation
 * EU 2022/2554).
 *
 * Extracted from {@see \App\Command\LoadDoraRtsItsFullCommand} so the same
 * Level-2 rows flow through BOTH paths:
 *   1. the standalone CLI command `app:load-dora-rts-its-full`, and
 *   2. the framework-loader registry path
 *      ({@see \App\Command\LoadDoraRequirementsCommand::loadRequirements()}),
 *      which is what the setup-wizard / admin "load framework" UI invokes.
 *
 * Before this extraction the Level-2 catalogue was ONLY reachable via the
 * manual CLI command — never the registry — so a fresh install loaded zero
 * RTS/ITS requirementIds and the `dora-rts-its_to_iso27001-2022` mapping
 * (whose `source` ids are `RTS-ICT-RMF-Art.*` etc.) had nothing to resolve
 * against. The MappingValidatorService then rejected the whole mapping file
 * ("source '…' not found in framework 'DORA'."), so the Level-2 depth was
 ...
 *   - 'ITS-Register-RT.XX.YY'       — CIR (EU) 2024/2956 ITS Register of Information templates
 ...
 *   - 'RTS-TPP-Policy-Art.X'        — CDR 2024/1773 RTS on TPP contractual arrangements policy
 ...
 *   - 'DORA-N.M' / 'DORA-RTS-*' / 'DORA-ITS-*' — populated by
 *     {@see \App\Command\LoadDoraRequirementsCommand}.
 *
 * IMPORTANT: This loader inserts the NEW Level-2 IDs only and never overwrites
 * Level-1 'DORA-N.M'. It is idempotent — safe to run repeatedly. Existing
 * Level-2 rows are skipped unless $update is true.
 *
 * RATIONALE-VALIDATION CAVEAT:
 *   The exact article-numbering of the ESAs Final Reports / CDRs / CIRs MAY have
 *   shifted between draft and final publication. Where the text below is taken
 *   from a draft, the row description carries the VALIDATION_HINT phrase.
 *   Operational consumers (compliance teams) MUST cross-check identifiers
 *   against the OJ-published version before relying on them for audit.
 */
final class DoraRtsItsCatalogueLoader
{
    public const VALIDATION_HINT = 'Anwender muss gegen die offizielle ESAs-Final-Report-Veroeffentlichung validieren.';
    ...
    /**
     * Persist the DORA Level-2 catalogue into the given framework.
     *
     * Does NOT flush — the caller owns the unit-of-work boundary so the
     * Level-1 + Level-2 rows can be flushed together.
     *
     * @return array{created: int, updated: int, skipped: int, total: int, blocks: int}
     */
    public function load(ComplianceFramework $framework, bool $update = false): array
    {
        ...
        foreach ($blocks as $blockName => $block) {
            ...
            foreach ($block['items'] as $reqId => $item) {
                ...
                $isNew = ($req === null);
                if (!$isNew && !$update) {
                    // Idempotent skip: existing Level-2 row, not updating.
                    $skipped++;
                    continue;
                }
                if ($isNew) {
    ...
    /**
     * Flat list of every Level-2 requirementId this loader produces.
     *
     * Used by the resolution-guard test to assert that every mapping `source`
     * id has a counterpart row, without needing a database.
     *
     * @return list<string>
     */
    public function requirementIds(): array
    {
        ...
        foreach ($this->getAllBlocks() as $block) {
            foreach (array_keys($block['items']) as $reqId) {
                $ids[] = (string) $reqId;
            }
        }
