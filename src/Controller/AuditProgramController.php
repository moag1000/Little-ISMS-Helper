...
/**
 * AuditProgramController — ISO 19011 §5.4 Jahresplan CRUD.
 *
 * RBAC: ROLE_AUDITOR or ROLE_ADMIN required for all read-actions.
 * Write actions (new/edit/approve) require ROLE_MANAGER or ROLE_ADMIN.
 * Module gate: `audits`.
 */
#[IsGranted('ROLE_AUDITOR')]
#[Route('/audit-program', name: 'app_audit_program_')]
class AuditProgramController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        // Group by year for the overview stats
        $byYear   = [];
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->auditLogger->logCreate(
                'AuditProgram',
                $program->getId(),
                ['name' => $program->getName(), 'year' => $program->getYear()],
            );
    ...
    #[IsGranted('ROLE_MANAGER')]
    #[IsCsrfTokenValid('approve-audit-program-{id}')]
    public function approve(Request $request, AuditProgram $auditProgram): Response
    {
        if ($redirect = $this->checkModuleActive('audits')) {
            return $redirect;
        }
        ...
        try {
            ...
            $auditProgram->setApprovedBy($user);
            $auditProgram->setApprovedAt(new DateTimeImmutable());
            $auditProgram->setUpdatedAt(new DateTimeImmutable());
            $this->entityManager->flush();

            $this->flashSuccess('audit_program.success.approved');
        } catch (\Throwable $e) {
            $this->flashError('audit_program.error.transition_failed');
        }
