...
| `fa-modal` (confirm + settings + wizard) | Canonical unified modal library. 3 modes: `confirm` (destructive/approval, 4 tones), `settings` (preferences/toggles), `wizard` (multi-step). Stimulus shell controller `fa-modal` + dispatcher `fa-modal-dispatcher`. Sub-controllers `fa-confirm` (type-to-confirm + cooldown) compose alongside via `data-controller="fa-modal fa-confirm"`. Open from JS via `this.dispatch('fa-modal:request-open', { detail: { id: 'modal-id' } })` — drop-in replacement for `new bootstrap.Modal()`. | `_fa_modal.html.twig` |
